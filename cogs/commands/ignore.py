...
class Ignore(commands.Cog):
  ...
  async def _ignore(self, ctx):
    if ctx.subcommand_passed is None:
      embed = discord.Embed(
          title="<:Ignore:1487128712477675602> Ignore Commands",
          description="Manage which commands, channels, and users are ignored by the bot.",
          color=0x134E5E
      )
      embed.set_author(name="CupidX Help Center", icon_url=self.bot.user.display_avatar.url)
      
      sub_cmds = (
          f"`{ctx.prefix}ignore command` - Manage ignored commands\n"
          f"`{ctx.prefix}ignore channel` - Manage ignored channels\n"
          f"`{ctx.prefix}ignore user` - Manage ignored users\n"
          f"`{ctx.prefix}ignore bypass` - Manage users who bypass ignores"
      )
      embed.add_field(name="Subgroups", value=sub_cmds, inline=False)
  ...
  async def _command(self, ctx):
      if ctx.subcommand_passed is None:
          embed = discord.Embed(
              ...
              description="Manage which commands are ignored in this server.",
              color=0x134E5E
          )
          embed.set_author(name="CupidX Help Center", icon_url=self.bot.user.display_avatar.url)
          
          sub_cmds = (
              f"`{ctx.prefix}ignore command add` - Ignore a specific command\n"
              f"`{ctx.prefix}ignore command remove` - Stop ignoring a command\n"
              f"`{ctx.prefix}ignore command show` - View ignored commands"
          )
  ...
  async def command_add(self, ctx: commands.Context, command_name: str):
      ...
      if not command:
          embed = discord.Embed(title="<:CupidXCross:1473996646873436336> Error", description=f"`{command_name}` is not a valid command.", color=0xFCD005)
      ...
      async with aiosqlite.connect(self.db_path) as db:
          ...
          if count[0] >= 25:
              embed = discord.Embed(description="You can only add up to 25 commands to the ignore list.", color=0xFCD005)
              await ctx.reply(embed=embed)
              return
          ...
          if result:
              embed = discord.Embed(description=f"`{command_name}` is already in the ignore commands list.", color=0xFCD005)
              await ctx.reply(embed=embed, mention_author=False)
          else:
  ...
  async def command_remove(self, ctx: commands.Context, command_name: str):
      ...
      async with aiosqlite.connect(self.db_path) as db:
          ...
          result = await cursor.fetchone()
          if not result:
              embed = discord.Embed(title="<:CupidXCross:1473996646873436336> Error", description=f"`{command_name}` is not in the ignore commands list.", color=self.color)
              await ctx.reply(embed=embed)
          else:
  ...
  async def command_show(self, ctx: commands.Context):
      async with aiosqlite.connect(self.db_path) as db:
          ...
          if not commands:
              embed = discord.Embed(description="No commands are currently ignored in this server.", color=0xFCD005)
              await ctx.reply(embed=embed, mention_author=False)
          else:
              entries = [f"`{command[0]}`" for command in commands]
              description = "\n".join(entries)
              embed = discord.Embed(title="Ignored Commands", description=description, color=0xFCD005)
              await ctx.reply(embed=embed, mention_author=False)
  ...
  async def _channel(self, ctx):
    if ctx.subcommand_passed is None:
      embed = discord.Embed(
          ...
          description="Manage which channels are ignored in this server.",
          color=0x134E5E
      )
      embed.set_author(name="CupidX Help Center", icon_url=self.bot.user.display_avatar.url)
      
      sub_cmds = (
          f"`{ctx.prefix}ignore channel add` - Ignore a specific channel\n"
          f"`{ctx.prefix}ignore channel remove` - Stop ignoring a channel\n"
          f"`{ctx.prefix}ignore channel show` - View ignored channels"
      )
      embed.add_field(name="Subcommands", value=sub_cmds, inline=False)
      embed.set_footer(text="cupidx HQ • cupidx", icon_url=ctx.author.display_avatar.url)
      await ctx.reply(embed=embed)
  ...
  async def channel_add(self, ctx: commands.Context, channel: discord.TextChannel):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if count[0] >= 30:
        embed = discord.Embed(title="<:notify:1487128417676689571> Access Denied", description="You can only add up to 30 channels to the ignore list.", color=self.color)
        await ctx.reply(embed=embed)
        return
      ...
      if result:
        embed = discord.Embed(title="<:CupidXCross:1473996646873436336> Error", description=f"{channel.mention} is already in the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed, mention_author=False)
      else:
        await db.execute("INSERT INTO ignored_channels (guild_id, channel_id) VALUES (?, ?)", (ctx.guild.id, channel.id))
        await db.commit()
        embed = discord.Embed(title="<:CupidXtick:1474025519531163772> Success", description=f"Successfully added {channel.mention} to the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def channel_remove(self, ctx: commands.Context, channel: discord.TextChannel):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if not result:
        embed = discord.Embed(title="<:CupidXCross:1473996646873436336> Error", description=f"{channel.mention} is not in the ignore channels list.", color=0xFCD005)
        await ctx.reply(embed=embed)
      else:
        await db.execute("DELETE FROM ignored_channels WHERE guild_id = ? AND channel_id = ?", (ctx.guild.id, channel.id))
        await db.commit()
        embed = discord.Embed(title="<:CupidXtick:1474025519531163772> Success", description=f"Successfully removed {channel.mention} from the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def _user(self, ctx):
    if ctx.subcommand_passed is None:
      embed = discord.Embed(
          title="🚫 Ignore User",
          description="Manage which users are ignored in this server.",
          color=0x134E5E
      )
      embed.set_author(name="CupidX Help Center", icon_url=self.bot.user.display_avatar.url)
      
      sub_cmds = (
          f"`{ctx.prefix}ignore user add` - Ignore a specific user\n"
          f"`{ctx.prefix}ignore user remove` - Stop ignoring a user\n"
          f"`{ctx.prefix}ignore user show` - View ignored users"
      )
      embed.add_field(name="Subcommands", value=sub_cmds, inline=False)
      embed.set_footer(text="cupidx HQ • cupidx", icon_url=ctx.author.display_avatar.url)
      await ctx.reply(embed=embed)
  ...
  async def user_add(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if count[0] >= 30:
        embed = discord.Embed(title="<:notify:1487128417676689571> Access Denied", description="You can only add up to 30 users to the ignore list.", color=self.color)
        await ctx.reply(embed=embed)
        return
      ...
      if result:
        embed = discord.Embed(title="<:CupidXCross:1473996646873436336>> Error", description=f"{user.mention} is already in the ignore users list.", color=self.color)
        await ctx.reply(embed=embed, mention_author=False)
      else:
        await db.execute("INSERT INTO ignored_users (guild_id, user_id) VALUES (?, ?)", (ctx.guild.id, user.id))
        await db.commit()
        embed = discord.Embed(title="<:CupidXtick:1474025519531163772> Success", description=f"Successfully added {user.mention} to the ignore users list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def user_remove(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if not result:
        embed = discord.Embed(title="<:CupidXCross:1473996646873436336> Error", description=f"{user.mention} is not in the ignore users list.", color=self.color)
        await ctx.reply(embed=embed)
      else:
        await db.execute("DELETE FROM ignored_users WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, user.id))
        await db.commit()
        embed = discord.Embed(title="<:CupidXtick:1474025519531163772> Success", description=f"Successfully removed {user.mention} from the ignore users list.", color=self.color)
  ...
  async def _bypass(self, ctx):
    if ctx.subcommand_passed is None:
      embed = discord.Embed(
          title="🚫 Ignore Bypass",
          description="Manage users who are allowed to bypass all ignore filters.",
          color=0x134E5E
      )
      embed.set_author(name="CupidX Help Center", icon_url=self.bot.user.display_avatar.url)
      
      sub_cmds = (
          f"`{ctx.prefix}ignore bypass add` - Grant ignore bypass to a user\n"
          f"`{ctx.prefix}ignore bypass remove` - Revoke ignore bypass from a user\n"
          f"`{ctx.prefix}ignore bypass show` - View all bypassed users"
      )
      embed.add_field(name="Subcommands", value=sub_cmds, inline=False)
      embed.set_footer(text="cupidx HQ • cupidx", icon_url=ctx.author.display_avatar.url)
      await ctx.reply(embed=embed)
  ...
  async def bypass_add(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if result:
        embed = discord.Embed(title="<:CupidXCross:1473996646873436336> Error", description=f"{user.mention} is already in the bypass users list.", color=self.color)
        await ctx.reply(embed=embed, mention_author=False)
      else:
        await db.execute("INSERT INTO bypassed_users (guild_id, user_id) VALUES (?, ?)", (ctx.guild.id, user.id))
        await db.commit()
        embed = discord.Embed(title="<:CupidXtick:1474025519531163772> Success", description=f"Successfully added {user.mention} to the bypass users list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def bypass_remove(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if not result:
        ...
        await db.commit()
        embed = discord.Embed(title="<:CupidXtick:1474025519531163772> Success", description=f"Successfully removed {user.mention} from the bypass users list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def bypass_show(self, ctx: commands.Context):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if not users:
        embed = discord.Embed(description="No users are currently bypassed in this server.", color=0xFCD005)
        await ctx.reply(embed=embed, mention_author=False)
      else:
        user_mentions = [ctx.guild.get_member(user_id).mention if ctx.guild.get_member(user_id) else f"User ID {user_id}" for user_id, in users]
        description = "\n".join(user_mentions)
