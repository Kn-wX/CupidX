...
class Games(Cog):
    ...
    @commands.command(name="tic-tac-toe",
    ...
    @commands.command(name="memory-game",
    ...
    async def _country_guesser(self, ctx: Context):
        if ctx.invoked_subcommand is None:
            embed = discord.Embed(
                ...
                description="Guess the name of the country and test your knowledge!",
                color=0x134E5E
            )
            embed.set_author(name="CupidX Help Center", icon_url=self.client.user.display_avatar.url)
            
            sub_cmds = (
                f"`{ctx.prefix}country-guesser start` - Start a new game (100 seconds)"
            )
