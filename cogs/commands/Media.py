...
class Media(commands.Cog):
    ...
    @commands.group(name="media", help="Setup Media channel, Media channel will not allow users to send messages other than media files.", invoke_without_command=True)
    @blacklist_check()
    @ignore_check()
    ...
    async def media(self, ctx):
        if ctx.subcommand_passed is None:
            embed = discord.Embed(
                title="<:nextramedia:1441435207021498598> Media Commands",
                description="Manage channels where only media files are allowed.",
                color=0x134E5E
            )
            embed.set_author(name="CupidX Help Center", icon_url=self.client.user.display_avatar.url)
            
            sub_cmds = (
                f"`{ctx.prefix}media setup` - Set a media-only channel\n"
                f"`{ctx.prefix}media remove` - Remove the media-only channel\n"
                f"`{ctx.prefix}media config` - View current media configuration\n"
                f"`{ctx.prefix}media bypass` - Manage bypass permissions"
            )
    ...
    async def bypass(self, ctx):
        if ctx.subcommand_passed is None:
            embed = discord.Embed(
                title="🖼️ Media Bypass",
                description="Manage users permitted to send text in media channels.",
                color=0x134E5E
            )
            embed.set_author(name="CupidX Help Center", icon_url=self.client.user.display_avatar.url)
            
            sub_cmds = (
                f"`{ctx.prefix}media bypass add` - Add a user to bypass list\n"
                f"`{ctx.prefix}media bypass remove` - Remove a user from bypass list\n"
                f"`{ctx.prefix}media bypass show` - View all bypassed users"
            )
            embed.add_field(name="Subcommands", value=sub_cmds, inline=False)
            embed.set_footer(text="cupidx HQ • cupidx", icon_url=ctx.author.display_avatar.url)
            await ctx.reply(embed=embed)
