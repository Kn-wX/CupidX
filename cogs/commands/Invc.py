...
class Invcrole(commands.Cog):
    ...
    async def vcrole(self, ctx):
        if ctx.subcommand_passed is None:
            embed = discord.Embed(
                title="🎤 VC Role Commands",
                description="Manage roles that are automatically given when users join voice channels.",
                color=0x134E5E
            )
            embed.set_author(name="CupidX Help Center", icon_url=self.bot.user.display_avatar.url)
            
            sub_cmds = (
                f"`{ctx.prefix}vcrole add` - Set a role to be given in VC\n"
                f"`{ctx.prefix}vcrole remove` - Remove the VC role\n"
                f"`{ctx.prefix}vcrole config` - View VC role configuration"
            )
            ...
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (ctx.guild.id,)) as cursor:
                ...
                if row:
                    embed = discord.Embed(title="⚠️ Access Denied",
                                          description=f"VC role is already set in this guild with the role {ctx.guild.get_role(row[0]).mention}.\nPlease **remove** it to add another one.", color=0xFCD005)
                    await ctx.reply(embed=embed)
                    return
            await db.execute('INSERT INTO vcroles (guild_id, role_id) VALUES (?, ?)', (ctx.guild.id, role.id))
            await db.commit()
            embed = discord.Embed(title="<:CupidXtick1:1474369967271968949> Success",
                                  description=f"VC role {role.mention} added for this guild.", color=0xFCD005)
            await ctx.reply(embed=embed)
    ...
    async def remove(self, ctx, role: discord.Role):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ? AND role_id = ?', (ctx.guild.id, role.id)) as cursor:
                row = await cursor.fetchone()
                if not row:
                    embed = discord.Embed(title="<:CupidXCross:1473996646873436336> Error",
                                          description="Given role is not set in VC role.", color=0xFCD005)
                    await ctx.send(embed=embed)
                    return
            await db.execute('DELETE FROM vcroles WHERE guild_id = ? AND role_id = ?', (ctx.guild.id, role.id))
            await db.commit()
            embed = discord.Embed(title="<:CupidXtick1:1474369967271968949> Success",
                                  description=f"VC role {role.mention} removed for this guild.", color=0xFCD005)
            await ctx.send(embed=embed)
    ...
    async def config(self, ctx):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (ctx.guild.id,)) as cursor:
                ...
                if not row:
                    embed = discord.Embed(title="<:CupidXCross:1473996646873436336> Error",
                                          description="VC role is not set in this guild.", color=0xFCD005)
                    await ctx.send(embed=embed)
                    return
                role = ctx.guild.get_role(row[0])
                embed = discord.Embed(title="VC Role Configuration",
                                      description=f"Current VC role in this guild is {role.mention}.", color=0xFCD005)
