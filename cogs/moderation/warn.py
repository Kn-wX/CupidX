...
class Warn(commands.Cog):
    ...
    async def warn(self, ctx, user: discord.Member = commands.parameter(description="The member to warn"), *, reason: str = commands.parameter(description="Reason for warning", default=None)):
        if user == ctx.author:
        ...
        try:
            ...
            body = (f"**Target User:** [{user}](https://discord.com/users/{user.id})\n"
            ...
            view = v2_card(f"Successfully Warned {user.name}", body)
            btn_view = WarnView(user=user, author=ctx.author)
    ...
    @commands.guild_only()
    @commands.has_permissions(moderate_members=True)
    async def clearwarns(self, ctx, user: discord.Member = commands.parameter(description="The member to clear warnings for")):
        try:
            await self.reset_warns(ctx.guild.id, user.id)
            view = v2_card("Warnings Cleared", f"All warnings have been cleared for **{user}** in this guild.")
            await ctx.send(view=view)
        except Exception as e:
            await ctx.send(f"An error occurred: {str(e)}")
