...
class Moderation(commands.Cog):
  ...
  async def enlarge(self, ctx,  emoji: Union[discord.Emoji, discord.PartialEmoji, str] = commands.parameter(description="The emoji to enlarge")):
    url = emoji.url
    await ctx.send(url)
  ...
  async def unlockall(self, ctx):
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
          ...
          async def button_callback(interaction: discord.Interaction):
              ...
              if interaction.user == ctx.author:
                  if interaction.guild.me.guild_permissions.manage_roles:
                      view = v2_card("In Progress", f"Unlocking all channels in **{ctx.guild.name}**...")
                      ...
                      view = v2_card("Success", f"Successfully Unlocked **{a}** Channels.")
          ...
          view = v2_card("Access Denied ⚠️", "Your role should be above my top role.")
  ...
  async def lockall(self, ctx):
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
          ...
          async def button_callback(interaction: discord.Interaction):
              ...
              if interaction.user == ctx.author:
                  if interaction.guild.me.guild_permissions.manage_roles:
                      view = v2_card("In Progress", f"Locking all channels in **{ctx.guild.name}**...")
                      await interaction.response.edit_message(view=view)
                      for channel in interaction.guild.channels:
                          try:
                      ...
                      view = v2_card("Success", f"Successfully locked **{a}** Channels.")
                      await interaction.channel.send(view=view)
                      return
                  else:
                    await interaction.response.edit_message(
  ...
  async def give(self, ctx, member: discord.Member = commands.parameter(description="The member to give the role to"), *, role: discord.Role = commands.parameter(description="The role to give")):
    ...
    if role >= ctx.guild.me.top_role:
        ...
        error.set_author(name="Error", icon_url=MOD_ERROR_ICON)
    ...
    if ctx.author != ctx.guild.owner and ctx.author.top_role <= member.top_role:
        ...
        error.set_author(name="Access Denied", icon_url=MOD_ERROR_ICON)
        error.set_footer(text=f"Requested by {ctx.author}",
                        icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        return await ctx.send(embed=error)
    ...
    try:
        if role not in member.roles:
            ...
            view = v2_card("Role Added", f"Successfully **added** role {role.mention} to {member.mention}.")
        else:
            await member.remove_roles(role, reason=f"Role removed by {ctx.author} (ID: {ctx.author.id})")
            view = v2_card("Role Removed", f"Successfully **removed** role {role.mention} from {member.mention}.")
        await ctx.send(view=view)
    except discord.Forbidden:
        view = v2_card("Error ⚠️", "I don't have permission to manage roles for this user!")
        await ctx.send(view=view)
    except Exception as e:
        view = v2_card("Error ⚠️", f"An unexpected error occurred: {str(e)}")
        await ctx.send(view=view)
  ...
  async def hideall(self, ctx):
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
          ...
          async def button_callback(interaction: discord.Interaction):
              ...
              if interaction.user == ctx.author:
                  if interaction.guild.me.guild_permissions.manage_roles:
                      view = v2_card("In Progress", f"Hiding all channels in **{ctx.guild.name}**...")
                      await interaction.response.edit_message(view=view)
                      for channel in interaction.guild.channels:
                          try:
                      ...
                      view = v2_card("Success", f"Successfully Hidden **{a}** Channel(s).")
                      await interaction.channel.send(view=view)
                      return
                  else:
                    await interaction.response.edit_message(
  ...
  async def unhideall(self, ctx):
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
          ...
          async def button_callback(interaction: discord.Interaction):
              ...
              if interaction.user == ctx.author:
                  if interaction.guild.me.guild_permissions.manage_roles:
                      view = v2_card("In Progress", f"Unhiding all channels in **{ctx.guild.name}**...")
                      await interaction.response.edit_message(view=view)
                      for channel in interaction.guild.channels:
                          try:
                      ...
                      view = v2_card("Success", f"Successfully Unhidden **{a}** Channel(s).")
                      await interaction.channel.send(view=view)
                      return
                  else:
                    await interaction.response.edit_message(
  ...
  @commands.max_concurrency(1, per=commands.BucketType.default, wait=False)
  @commands.guild_only()
  async def _prefix(self, ctx: commands.Context, prefix: str = commands.parameter(description="The new prefix")):
      if not prefix:
      ...
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
          ...
          msg = (
              f"Changed Prefix For this guild to `{prefix}`\n\n"
              f"New Prefix for **{ctx.guild.name}** is : `{prefix}`\n"
              f"Use `{prefix}help` For More."
          )
          view = v2_card("Prefix Updated <:tick:1327829594954530896>", msg)
  ...
  async def clone(self, ctx: commands.Context, channel: discord.TextChannel = commands.parameter(description="The channel to clone")):
    ...
    try:
        ...
        await channel.clone()
        view = v2_card("Success", f"{channel.name} has been successfully cloned.")
        await ctx.send(view=view)
    except discord.Forbidden:
        view = v2_card("Error ⚠️", "I don't have permission to clone channels!")
        await ctx.send(view=view)
    except Exception as e:
        view = v2_card("Error ⚠️", f"An error occurred: {str(e)}")
        await ctx.send(view=view)
  ...
  async def changenickname(self, ctx: commands.Context, member: discord.Member = commands.parameter(description="The member to rename"), *, name: str = commands.parameter(description="The new nickname (leave empty to reset)", default=None)):
    ...
    try:
        ...
        msg = f"Successfully changed nickname of {member.mention} to **{name}**." if name else f"Successfully cleared nickname of {member.mention}."
        view = v2_card("Nickname Updated", msg)
        await ctx.send(view=view)
    except discord.Forbidden:
        view = v2_card("Error ⚠️", "I don't have permission to manage this user's nickname!")
        await ctx.send(view=view)
    except Exception as e:
        view = v2_card("Error ⚠️", f"An error occurred: {str(e)}")
        await ctx.send(view=view)
    ...
    async def button_callback(interaction: discord.Interaction):
      if interaction.user == ctx.author:
        if interaction.guild.me.guild_permissions.manage_channels:
          ...
          message = f"Channel has been Successfully nuked by **{ctx.author}**"
          view = v2_card("Channel Nuked", message)
          await newchannel.send(view=view)
        else:
          await interaction.response.edit_message(
            content=
  ...
  async def _slowmode(self, ctx: commands.Context, seconds: int = commands.parameter(description="Slowmode duration (seconds)", default=0)):
    if seconds > 120:
      embed=discord.Embed(description="Slowmode can not be over 2 minutes",
                            color=self.color)
      embed.set_author(name="Error", icon_url=MOD_ERROR_ICON)
      embed.set_footer(text=f"Requested by {ctx.author}",
                        icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
      return await ctx.send(embed=embed)
    if seconds == 0:
      await ctx.channel.edit(slowmode_delay=seconds)
      view = v2_card("Slowmode", "Slowmode is disabled.")
      await ctx.send(view=view)
    else:
      await ctx.channel.edit(slowmode_delay=seconds)
      view = v2_card("Slowmode Activated", f"Successfully Set slowmode to **{seconds}** seconds.")
      await ctx.send(view=view)
  ...
  async def _unslowmode(self, ctx: commands.Context):
    ...
    view = v2_card("Slowmode Disabled", "Successfully disabled slowmode.")
    await ctx.send(view=view)
  ...
  async def delsticker(self, ctx: commands.Context, *, name=commands.parameter(description="The sticker name or ID to delete", default=None)):
  ...
  @commands.cooldown(1, 3, commands.BucketType.user)
  @commands.has_permissions(manage_emojis=True)
  async def delemoji(self, ctx, emoji: str = commands.parameter(description="The emoji to delete", default=None)):
  ...
  async def roleicon(self, ctx: commands.Context, role: discord.Role = commands.parameter(description="The role to edit"), *, icon: Union[discord.Emoji, discord.PartialEmoji, str] = commands.parameter(description="New icon (emoji, url, or attachment)", default=None)):
    ...
    if role.position >= ctx.guild.me.top_role.position:
        ...
        error_embed.set_author(name="Error", icon_url=MOD_ERROR_ICON)
    ...
    async def button_callback(interaction: discord.Interaction):
      ...
      if interaction.user == ctx.author:
        if interaction.guild.me.guild_permissions.ban_members:
          view = v2_card("In Progress", "Unbanning All Banned Members...")
          await interaction.response.edit_message(view=view)
          ...
          view = v2_card("Success", f"Successfully Unbanned **{a}** Members.")
          await interaction.channel.send(view=view)
        else:
          view = v2_card("Error ⚠️", "I am missing the `ban members` permission in this server.")
          await interaction.response.edit_message(view=view)
      else:
        await interaction.response.send_message("Uh oh! That message doesn't belong to you.", ephemeral=True)

    async def button1_callback(interaction: discord.Interaction):
      if interaction.user == ctx.author:
        view = v2_card("Cancelled", "Cancelled, I will not unban anyone.")
        await interaction.response.edit_message(view=view)
      else:
        await interaction.response.send_message("Uh oh! That message doesn't belong to you.", ephemeral=True)
  ...
  @commands.max_concurrency(1, per=commands.BucketType.default, wait=False)
  @commands.guild_only()
  async def auditlog(self, ctx, limit: int = commands.parameter(description="Number of entries to fetch (max 30)")):
    if limit >= 31:
      await ctx.reply(
    ...
    async for entry in ctx.guild.audit_logs(limit=limit):
      idk.append(f"**User**: `{entry.user}` | **Action**: `{entry.action}` | **Target**: `{entry.target}`\n")
    
    log_text = "".join(idk).replace("AuditLogAction.", "")
    view = v2_card(f"Audit Logs: {ctx.guild.name}", log_text)
