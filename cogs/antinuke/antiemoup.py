"""
AntiEmojiUpdate — Cypher Speed + GOD BASE
...
class AntiEmojiUpdate(AntinukeBase):
    ...
    async def on_guild_emojis_update(self, guild: discord.Guild, before, after):
        ...
        await self.punish_and_notify(
            guild, executor, "Emoji Update", f"{'Banned' if mass else 'Kicked'} executor",
            punishment_coro=action(guild, executor, reason=f"Anti-EmojiUpdate | {'MASS ATTACK' if mass else 'Unwhitelisted User'}"),
            reason=f"Unwhitelisted user **updated Emoji(s)**. Executor was {'**banned**' if mass else '**kicked**'}.\nAttack type: {'🚨 Mass Attack' if mass else 'Single Event'}",
            is_mass_attack=mass,
        )
